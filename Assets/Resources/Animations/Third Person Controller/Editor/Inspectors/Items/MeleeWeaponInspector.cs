        ...
        public override void OnInspectorGUI()
        {
            ...
            if ((m_AttackFoldout = EditorGUILayout.Foldout(m_AttackFoldout, "Attack Options", InspectorUtility.BoldFoldout))) {
                ...
                if (attackType.enumValueIndex == (int)MeleeWeapon.HitType.Hitscan) {
                    ...
                    if (collider == null) {
                        EditorGUILayout.HelpBox("A collider must exist on the weapon if using the Collision attack type.", MessageType.Error);
                    }
                    // As does a Rigidbody.
                    var rigidbody = meleeWeapon.GetComponent<Rigidbody>();
                    if (rigidbody == null || rigidbody.isKinematic == false) {
                        EditorGUILayout.HelpBox("A kinematic Rigidbody must exist on the weapon if using the Collision attack type.", MessageType.Error);
                    }
                }
