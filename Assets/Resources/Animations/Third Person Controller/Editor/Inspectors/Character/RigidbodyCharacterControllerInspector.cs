    ...
    public class RigidbodyCharacterControllerInspector : InspectorBase
    {
        ...
        private void OnEnable()
        {
            ...
            for (int i = 0; i < assemblies.Length; ++i) {
                ...
                for (int j = 0; j < assemblyTypes.Length; ++j) {
                    ...
                    // Ignore the Third Person Controller base classes if there is a wrapper class.
                    if (!string.IsNullOrEmpty(assemblyTypes[j].Namespace) && assemblyTypes[j].Namespace.Equals("Opsive.ThirdPersonController.Abilities")) {
                        var hasWrapper = false;
                        for (int k = 0; k < assemblyTypes.Length; ++k) {
                            if (assemblyTypes[j].Name.Equals(assemblyTypes[k].Name) &&
                                !string.IsNullOrEmpty(assemblyTypes[k].Namespace) && assemblyTypes[k].Namespace.Equals("Opsive.ThirdPersonController.Wrappers.Abilities")) {
                                hasWrapper = true;
                                break;
                            }
                        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if ((m_MovementFoldout = EditorGUILayout.Foldout(m_MovementFoldout, "Movement Options", InspectorUtility.BoldFoldout))) {
                ...
                EditorGUILayout.PropertyField(PropertyFromName(serializedObject, "m_IndependentLook"));
            ...
            if ((m_AbilityFoldout = EditorGUILayout.Foldout(m_AbilityFoldout, "Abilities", InspectorUtility.BoldFoldout))) {
                if (m_ReorderableAbilityList == null) {
                    ...
                    m_ReorderableAbilityList = new ReorderableList(serializedObject, m_Abilities, true, false, true, true);
        ...
        private void OnAbilityListRemove(ReorderableList list)
        {
            ...
            list.index = -1;
            m_Controller.SelectedAbility = -1;
            EditorUtility.SetDirty(m_Controller);
        }
